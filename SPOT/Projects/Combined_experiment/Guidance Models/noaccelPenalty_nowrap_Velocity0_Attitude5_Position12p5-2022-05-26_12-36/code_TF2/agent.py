...
class Agent:
    ...
    def create_summary_functions(self):
        ...
        self.timestep_number_placeholder                       = tf.compat.v1.placeholder(tf.float32)
        self.episode_reward_placeholder                        = tf.compat.v1.placeholder(tf.float32)
        self.combined_angular_momentum_if_captured_placeholder = tf.compat.v1.placeholder(tf.float32)
        self.combined_angular_rate_if_captured_placeholder     = tf.compat.v1.placeholder(tf.float32)
        timestep_number_summary                                = tf.compat.v1.summary.scalar("Agent_" + str(self.n_agent) + "/Number_of_timesteps", self.timestep_number_placeholder)
        self.target_angular_velocity                           = tf.compat.v1.placeholder(tf.float32)
        episode_reward_summary                                 = tf.compat.v1.summary.scalar("Agent_" + str(self.n_agent) + "/Episode_reward", self.episode_reward_placeholder)
        combined_angular_momentum_if_captured_summary          = tf.compat.v1.summary.scalar("Agent_" + str(self.n_agent) + "/Captured_combined_angular_momentum", self.combined_angular_momentum_if_captured_placeholder)
        combined_angular_rate_if_captured_summary              = tf.compat.v1.summary.scalar("Agent_" + str(self.n_agent) + "/Captured_combined_angular_rate", self.combined_angular_rate_if_captured_placeholder)
        
        self.regular_episode_summary_docked                    = tf.compat.v1.summary.merge([timestep_number_summary, episode_reward_summary, combined_angular_momentum_if_captured_summary, combined_angular_rate_if_captured_summary])
        self.regular_episode_summary_not_docked                = tf.compat.v1.summary.merge([timestep_number_summary, episode_reward_summary])

        # If this is agent 1, the agent who will also test performance, additionally log the reward
        if self.n_agent == 1:
            test_time_episode_reward_summary              = tf.compat.v1.summary.scalar("Test_agent/Episode_reward", self.episode_reward_placeholder)
            test_time_timestep_number_summary             = tf.compat.v1.summary.scalar("Test_agent/Number_of_timesteps", self.timestep_number_placeholder)
            combined_angular_momentum_if_captured_summary = tf.compat.v1.summary.scalar("Test_agent/Captured_combined_angular_momentum", self.combined_angular_momentum_if_captured_placeholder) 
            combined_angular_rate_if_captured_summary     = tf.compat.v1.summary.scalar("Test_agent/Captured_combined_angular_rate", self.combined_angular_rate_if_captured_placeholder) 
            test_time_target_angular_velocity_summary     = tf.compat.v1.summary.scalar("Test_agent/Target_initial_angular_velocity", self.target_angular_velocity)
            
            self.test_time_episode_summary_docked         = tf.compat.v1.summary.merge([test_time_episode_reward_summary, test_time_timestep_number_summary, combined_angular_momentum_if_captured_summary, combined_angular_rate_if_captured_summary, test_time_target_angular_velocity_summary])
            self.test_time_episode_summary_not_docked     = tf.compat.v1.summary.merge([test_time_episode_reward_summary, test_time_timestep_number_summary, test_time_target_angular_velocity_summary])


    def build_actor(self):
        # Generate the actor's policy neural network
        agent_name = 'agent_' + str(self.n_agent) # agent name 'agent_3', for example
        self.state_placeholder = tf.compat.v1.placeholder(dtype = tf.float32, shape = [None, Settings.OBSERVATION_SIZE], name = 'state_placeholder') # the * lets Settings.OBSERVATION_SIZE be not restricted to only a scalar
