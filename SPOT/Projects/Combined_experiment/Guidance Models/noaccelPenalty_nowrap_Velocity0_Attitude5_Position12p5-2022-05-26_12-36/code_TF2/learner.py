...
#            self.importance_sampling_weights_placeholder = tf.placeholder(dtype = tf.float32, shape =  Settings.MINI_BATCH_SIZE, name = "importance_sampling_weights_placeholder") # [PRIORITY_REPLAY_BUFFER only] Holds the weights that are used to remove bias from priority sampling
            ...
            self.state_placeholder                       = tf.compat.v1.placeholder(dtype = tf.float32, shape = [None, Settings.OBSERVATION_SIZE], name = "state_placeholder") # the '*' unpacks the OBSERVATION_SIZE list (incase it's pixels of higher dimension)
            self.action_placeholder                      = tf.compat.v1.placeholder(dtype = tf.float32, shape = [None, Settings.ACTION_SIZE], name = "action_placeholder") # placeholder for actions
            self.target_bins_placeholder                 = tf.compat.v1.placeholder(dtype = tf.float32, shape = [None, Settings.NUMBER_OF_BINS], name = "target_bins_placeholder") # Bin values of target network with Bellman update applied
            self.target_q_distribution_placeholder       = tf.compat.v1.placeholder(dtype = tf.float32, shape = [None, Settings.NUMBER_OF_BINS], name = "target_q_distribution_placeholder")  # Future q-distribution from target critic
            self.dQ_dAction_placeholder                  = tf.compat.v1.placeholder(dtype = tf.float32, shape = [Settings.MINI_BATCH_SIZE, Settings.ACTION_SIZE], name = "dQ_dAction_placeholder") # Gradient of critic predicted value with respect to input actions
            self.importance_sampling_weights_placeholder = tf.compat.v1.placeholder(dtype = tf.float32, shape =  None, name = "importance_sampling_weights_placeholder") # [PRIORITY_REPLAY_BUFFER only] Holds the weights that are used to remove bias from priority sampling
    ...
    def create_summary_functions(self):
        ...
        with tf.compat.v1.variable_scope("Logging_Learning"):
            ...
            self.iteration_loss_summary = tf.compat.v1.summary.scalar("Loss", self.iteration_loss_placeholder)
            self.iteration_summary = tf.compat.v1.summary.merge([self.iteration_loss_summary])
