        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            if Settings.LEARN_FROM_PIXELS:
                ...
                for i, conv_layer_settings in enumerate(Settings.CONVOLUTIONAL_LAYERS):
                    self.layer = tf.compat.v1.layers.conv2d(inputs = self.layer,
                ...
                self.layer = tf.compat.v1.layers.flatten(self.layer) 
            
            
            # Building fully-connected layers
            for i, number_of_neurons in enumerate(Settings.ACTOR_HIDDEN_LAYERS):
                self.layer = tf.compat.v1.layers.dense(inputs = self.layer,
            ...
            self.actions_out_unscaled = tf.compat.v1.layers.dense(inputs = self.layer,
            ...
            self.parameters = tf.compat.v1.trainable_variables(scope = self.scope)
    ...
    def generate_training_function(self, dQ_dAction):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            with tf.compat.v1.variable_scope('Training'):
                ...
                self.optimizer = tf.compat.v1.train.AdamOptimizer(Settings.ACTOR_LEARNING_RATE)
                ...
                self.actor_gradients = tf.gradients(ys=self.action_scaled, xs=self.parameters, grad_ys=-dQ_dAction)  
    ...
    def __init__(self, state, action, scope):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            if Settings.LEARN_FROM_PIXELS:            
                # Build convolutional layers
                for i, conv_layer_settings in enumerate(Settings.CONVOLUTIONAL_LAYERS):
                    self.state_side = tf.compat.v1.layers.conv2d(inputs = self.state_side,
                                                       activation = tf.nn.relu,
                ...
                self.state_side = tf.compat.v1.layers.flatten(self.state_side) 
            ...
            for i, number_of_neurons in enumerate(Settings.CRITIC_HIDDEN_LAYERS):
                self.state_side = tf.compat.v1.layers.dense(inputs = self.state_side,
                                                  units = number_of_neurons,
                                                  activation = None,
            ...
            for i, number_of_neurons in enumerate(Settings.CRITIC_HIDDEN_LAYERS[1:]):
                self.action_side = tf.compat.v1.layers.dense(inputs = self.action_side,
                                                   units = number_of_neurons,
                                                   activation = None,
            ...
            self.q_distribution_logits = tf.compat.v1.layers.dense(inputs = self.layer,
            ...
            self.bins = tf.linspace(Settings.MIN_V, Settings.MAX_V, Settings.NUMBER_OF_BINS)
            ...
            self.dQ_dAction = tf.gradients(ys=self.q_distribution, xs=self.action, grad_ys=self.bins) # also known as action gradients
    ...
    def generate_training_function(self, target_q_distribution, target_bins, importance_sampling_weights):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            with tf.compat.v1.variable_scope('Training'):
                ...
                self.optimizer = tf.compat.v1.train.AdamOptimizer(Settings.CRITIC_LEARNING_RATE)               
                ...
                self.loss = tf.nn.softmax_cross_entropy_with_logits(logits = self.q_distribution_logits, labels = tf.stop_gradient(projected_target_distribution))
                ...
                self.mean_loss = tf.reduce_mean(input_tensor=self.weighted_loss)
